...
static u_char *initial_kern = (u_char*)0x7f00; // the boot loader load the kernel to 0x7d00 by int 0x13
static int kernel_sectors = 50;
...
cmain(void)
{
	...
	show_tag();
	if(*(u_int*)initial_kern != 0x464C457F)	// \x7F ELF in little endian
	;
	//	goto bad;

	// look at ELF header - ignores ph flags
	elf = (struct Elf*)initial_kern;
	entry = elf->e_entry;
	ph = (struct Proghdr*)(initial_kern+elf->e_phoff);
...
readseg(u_int va, u_int count, u_int offset)
{
	...
	for(i=0; i<count; i++){
		...
		for(j = 0; j < 512; j++)
		{
			*((u_char*)va + j) = *((initial_kern + i*512) + j);
		}
